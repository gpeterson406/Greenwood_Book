...
the rules used to this point - it doesn't have a formula interface.
...
https://ggplot2.tidyverse.org/ and the related book by Hadley Wickham, who now
works for R-studio. We could have used ``ggplot2`` to make every graph in the 
book, but elected to focus on the simpler formula interface that few of these
functions use. For now, I am going to use it to make Figure
...
not happening much for the subjects in the control group. 
...
```{r Figure9-13,fig.cap="(ref:fig9-13)",fig.height=5,fig.width=10,echo=F}
require(effects)
plot(allEffects(lme_int),multiline=T,ci.style="bars")
```
