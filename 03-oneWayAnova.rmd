...
x <- matrix(c(row1,row2,row3,row4,row5,row6, row7),ncol=3,byrow=TRUE)
m <- as.data.frame(x)
...
```{r Figure3-5,fig.cap="(ref:fig3-5)",echo=F,fig.height=5,fig.width=5}
set.seed(123)
B <- 1000
...
The p-value from the $F$-distribution is `r round(orig_anova[1,5],7)` so we can report it^[Any further claimed precision is an exaggeration and eventually we might see p-values that approach the precision of the computer at 2.2e-16 and anything below 0.0001 should just be reported as being below 0.0001. Also note the way that R represents small or extremely large numbers using scientific notation such as `3e-4` which is $3 \cdot 10^{-4} = 0.0003$.] as a p-value < 0.0001. \index{F-distribution} \index{p-values!small} We can 
...
```{r Figure3-7,fig.cap="(ref:fig3-7)",echo=F,fig.height=5,fig.width=5}
tibble(Tstar) %>%  ggplot(aes(x = Tstar)) + 
  geom_histogram(aes(y = ..ncount..), bins = 20, col = 1, fill = "skyblue") +
  stat_bin(aes(y = ..ncount.., label = ..count..), bins = 20, geom = "text") + 
  geom_density(aes(y = ..scaled..)) + theme_bw() + labs(y = "Density") +
...
```{r Figure3-10, fig.cap="(ref:fig3-10)",fig.height=5,fig.width=5}
par(mfrow=c(1,2))
...
#t(4)
y3 <- scale(rt(75,df=3)) #Overwrite with one from a different seed using following read-in, but not impact other random results below
y3 <- read_csv("simy3.csv")$y3
#Also available with y3 <- read_csv("http://www.math.montana.edu/courses/s217/documents/simy3.csv")$y3
#Uniform
y4 <- scale(runif(75,0,1))

tp <- tibble(y1, y2, y3, y4)

qy1p <- tp %>% ggplot(aes(sample = y1)) + geom_qq() + stat_qq_line() + theme_bw() +
  labs(title ="(a) QQ-Plot of y1")
d1p <- tp %>% ggplot(mapping = aes(y1)) + 
  geom_density(col = "darkred") + 
  labs(title = "(b) Density plot of y1", 
       y = "Density", 
       x = "") + theme_bw()

qy2p <- tp %>% ggplot(aes(sample = y2)) + geom_qq() + stat_qq_line() + theme_bw() +
  labs(title ="(c) QQ-Plot of y2")
d2p <- tp %>% ggplot(mapping = aes(y2)) + 
  geom_density(col = "darkred") + 
  labs(title = "(d) Density plot of y2", 
       y = "Density", 
       x = "") + theme_bw()


qy3p <- tp %>% ggplot(aes(sample = y3)) + geom_qq() + stat_qq_line() + theme_bw() +
  labs(title ="(e) QQ-Plot of y3")
d3p <- tp %>% ggplot(mapping = aes(y3)) + 
  geom_density(col = "darkred") + 
  labs(title = "(f) Density plot of y3", 
       y = "Density", 
       x = "") + theme_bw()

qy4p <- tp %>% ggplot(aes(sample = y4)) + geom_qq() + stat_qq_line() + theme_bw() +
  labs(title ="(g) QQ-Plot of y4")
d4p <- tp %>% ggplot(mapping = aes(y4)) + 
  geom_density(col = "darkred") + 
  labs(title = "(h) Density plot of y4", 
       y = "Density", 
       x = "") + theme_bw()

grid.arrange(qy1p, d1p, qy2p, d2p, qy3p, d3p, qy4p, d4p, ncol = 2)
```
...
qy5p <- tp2 %>% ggplot(aes(sample = y5)) + geom_qq() + stat_qq_line() + theme_bw() +
  labs(title ="(i) QQ-Plot of y5")
d5p <- tp2 %>% ggplot(mapping = aes(y5)) + 
  geom_density(col = "purple4") + 
  labs(title = "(j) Density plot of y5", 
       y = "Density", 
       x = "") + theme_bw()

qy6p <- tp2 %>% ggplot(aes(sample = y6)) + geom_qq() + stat_qq_line() + theme_bw() +
  labs(title ="(k) QQ-Plot of y6")
d6p <- tp2 %>% ggplot(mapping = aes(y6)) + 
  geom_density(col = "purple4") + 
  labs(title = "(j) Density plot of y6", 
       y = "Density", 
       x = "") + theme_bw()

grid.arrange(qy5p, d5p, qy6p, d6p, ncol = 2)
```
...
par(mfrow=c(1,1))
plot(allEffects(m2), rotx = 45) #rotx = 45 useful with many levels to read text
```
...
* **Tobs ``<-`` anova(lm(`r colFmt("Y","red")` ~ `r colFmt("X","red")`, data = `r colFmt("DATASETNAME","red")`))[1,4]; Tobs  
    B ``<-`` 1000  
    Tstar ``<-`` matrix(NA, nrow = B)  
    for (b in (1:B)){  
      Tstar[b] ``<-`` anova(lm(`r colFmt("Y","red")` ~ `r shuffle(colFmt("X","red"))`, data = `r colFmt("DATASETNAME","red")`))[1,4]
    }
    pdata(Tstar, Tobs, lower.tail = F)[[1]]**
