...
legend("topleft",lwd=c(2,2,2,3),title="Groups",legend=c("Control","T1","T2","T3"),lty=c(2,1,2,3),col=inferno(5)[-5])
```
...
plot(allEffects(head2), multiline = T, ci.style = "bands", grid = T, lty = c(1:4), lwd = 2)
```
...
```{r Figure8-42,fig.cap="(ref:fig8-42)",echo=F,fig.height=10,fig.width=10}
library(coneproj)
data(FEV)
FEV <- as_tibble(FEV)
...
```{r Figure8-44,fig.cap="(ref:fig8-44)",fig.height=5,fig.width=10}
...
```{r Figure8-45,fig.cap="(ref:fig8-45)",echo=F, fig.height=7, fig.width=10,echo=F,warning=F,message=F}
...
```{r Figure8-46,fig.cap="(ref:fig8-46)",fig.height=5,fig.width=10}
fm1R$coefficients
vif(fm1R)
confint(fm1R)
...
* **`r colFmt("DATASETNAME","red")` %>% ggplot(mapping = aes(x = `r colFmt("x","red")`, y = `r colFmt("y","red")`)) + geom_point() + geom_smooth(method = "lm")**
    ...
    * Add **color = `r colFmt("groupfactor","red")`** to aesthetic to color points and lines based on a grouping variable. 
    
    * Add **+ facet_grid(cols = vars(`r colFmt("groupfactor","red")`))** to facet by groups. \index{\texttt{facet_grid()}|textbf}
