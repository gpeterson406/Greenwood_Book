...
\indent To use the ``beanplot`` function \index{R functions!\textbf{beanplot}} we need to install and then load the ``beanplot`` 
...
Now if we remake the boxplots and beanplots, \index{beanplot} they only contain results for 
...
where they are most useful. For hypothesis testing, we will use permutations \index{permutation}
...
```{r Figure2-12, fig.cap="(ref:fig2-12)",echo=F,fig.width=10,fig.height=5}
par(mfrow=c(1,2))
hist(Tstar, labels=T,ylim=c(0,250))
abline(v=c(-1,1)*Tobs, lwd=2, col="red")
plot(density(Tstar), main="Density curve of Tstar")
...
```{r Figure2-13, echo=FALSE, fig.cap ="(ref:fig2-13)",fig.width=7,fig.height=4}
...
diagnostics later, but the boxplots and beanplots \index{beanplot} are a good place to start to 
...
2. Check conditions for the procedure being used
    ...
    * **Similar distribution condition**: Based on the side-by-side boxplots and
    beanplots \index{beanplot}, it appears that both groups have slightly left-skewed 
...
confidence level. \index{random sampling}]. The other assumptions are all the same as for the hypothesis 
...
data=`r colFmt("DATASETNAME","red")`, log="", col="lightblue", method="jitter")**
    ...
    * Makes a beanplot of a variable named Y for groups in X from the data set. \index{beanplot}
...
2.2. Make a beanplot \index{beanplot} and a boxplot of ``daysanysub`` ~ ``sex`` using the 
...
statistic you are using and include a discussion of the scope of inference. \index{scope of inference} 
