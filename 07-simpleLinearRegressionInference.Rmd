...
```{r Figure7-12,fig.cap="(ref:fig7-12)",fig.height=5,fig.width=10}
par(mfrow=c(1,2))
...
require(carData)
data(UN)
UN<-as.tibble(carData::UN) #Needed to find the correct UN data set from car
par(mfrow=c(2,2))
...
ppgdp1<-seq(from=30,to=42416,length.out=100)
...
```{r Figure7-24,fig.cap="(ref:fig7-24)",echo=F,fig.height=5,fig.width=10}
temp1<-lm(meanmax~Year, data=bozemantemps)
...
* **plot(allEffects(`r colFmt("MODELNAME","red")`))**
    ...
    * Provides a term-plot of the estimated regression line with 95% confidence
    interval for the mean. \index{\texttt{plot(allEffects())}|textbf}
