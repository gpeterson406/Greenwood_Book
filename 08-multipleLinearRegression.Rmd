...
* **Multicollinearity in MLR models:**
    ...
    corrplot.mixed(cor(snotel_s %>% slice(-c(9,22)) %>% select(3:6)), upper.col = c(1, "orange"), lower.col = c(1, "orange"))
...
* **`r colFmt("DATASETNAME","red")` %>% ggplot(mapping = aes(x = `r colFmt("x","red")`, y = `r colFmt("y","red")`)) + geom_point() + geom_smooth(method = "lm")**
    ...
    * Add **+ facet_grid(cols = vars(`r colFmt("groupfactor","red")`))** to facet by groups. \index{\texttt{facet\_grid()}|textbf}
