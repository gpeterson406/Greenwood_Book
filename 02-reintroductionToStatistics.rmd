...
``geom_histogram``, its aesthetic is modified using the (criptic^[I admit that there are parts of the logic of using `ggplot` that are confusing to me and this is one of them - but I learned to plot in R before `ggplot2`, but have been growing fonder and fonder of this way of working. Now instead of searching the internet, I will just get to search my book for this code.]) code of `(y = ..density..)`. The 
density curve is added to the histogram using the `geom_density`, producing the result in Figure \@ref(fig:Figure2-2) with 
added modifications for filling the density curve but using `alpha = 0.1` to make the density curve fill transparent (alpha values range between 0 and 1 with lower values providing more transparency). You can see how the density curve 
...
```{r Figure2-5, fig.cap="(ref:fig2-5)",echo=F, fig.width=6, fig.height=4}
library(yarrr)
set.seed(12345)
...
management once the data set is loaded. It is also a better scientific process to do as much of your data management within R as possible so that your steps in managing the data are fully documented and reproducible. Highlighting and clicking in spreadsheet programs is a dangerous way to work and can be impossible to recreate steps that were taken from initial data set to the version that was analyzed. In R, we could identify the rows that contain the observations we want to retain and just extract those rows, but this is hard with over five thousand observations. The `filter` function (also an option in some functions) is the best way to be able to focus on observations that meet a particular condition, we can "filter" the data set to retain just those rows. The `filter` function takes the data set via the pipe operate and then we need to define the condition we want to meet to retain those rows. Specifically, we need to define the variable we want to work with, `Condition`, and then request rows that meet a condition (are `%in%`) and the aspects that meet that condition (here by concatenating the two levels of "casual" and "commute"), leading to code of:

```
dd %>% filter(Condition %in% c("casual","commute"))
```

We would actually want to save that new filtered data set into a new tibble for future work, so we can use the following to save the reduced data set into `ddsub`:
...
```{r Figure2-7, echo=FALSE, fig.cap ="(ref:fig2-7)",out.width="100%",fig.align="center"}
...
```{r Figure2-8, fig.cap="(ref:fig2-8)",echo=F, fig.width=7, fig.pos="t"}
par(mfrow=c(1,2))
...
```{r Figure2-9, fig.cap="(ref:fig2-9)",echo=F,fig.width=5,fig.height=5,cache=F}
B <- 1000
set.seed(1234)
Tstar <- matrix(NA, nrow=B)
for (b in (1:B)){
...
Figure \@ref(fig:Figure2-10), using the ``geom_vline`` \index{texttt{geom_vline}} function to draw a vertical 
...
```{r Figure2-10, fig.cap="(ref:fig2-10)",echo=F,fig.width=5,fig.height=5,cache=F}
Tobs <- -25.933
tibble(Tstar) %>%  ggplot(aes(x = Tstar)) + 
  geom_histogram(aes(y = ..ncount..), bins = 15, col = 1, fill = "skyblue", center = 0) +
  stat_bin(aes(y = ..ncount.., label = ..count..), bins = 15, geom = "text") + 
...
```{r Figure2-11, fig.cap="(ref:fig2-11)",echo=F,fig.width=5,fig.height=5,cache=F}
tibble(Tstar) %>%  ggplot(aes(x = Tstar)) + 
  geom_histogram(aes(y = ..ncount..), bins = 15, col = 1, fill = "skyblue", center = 0) +
  stat_bin(aes(y = ..ncount.., label = ..count..), bins = 15, geom = "text") + 
  geom_density(aes(y = ..scaled..)) + theme_bw() + labs(y = "Density") +
...
```{r Figure2-12, fig.cap="(ref:fig2-12)",echo=F,out.width="100%",fig.align="center"}
...
```{r Figure2-13, fig.cap="(ref:fig2-13)",echo=F,fig.width=5,fig.height=5}
tibble(Tstar) %>%  ggplot(aes(x = Tstar)) + 
  geom_histogram(aes(y = ..ncount..), bins = 15, col = 1, fill = "skyblue", center = 0) +
  stat_bin(aes(y = ..ncount.., label = ..count..), bins = 15, geom = "text") + 
  geom_density(aes(y = ..scaled..)) + theme_bw() + labs(y = "Density") +
...
legend("left",legend=c("t(2)","t(8)","t(20)","Normal"),col=c("black","grey","black","blue"),lty=c(1,1,1,2),lwd=c(4,3,2,2),cex=0.75)
```
...
```{r Figure2-22, fig.cap="(ref:fig2-22)",echo=T,fig.width=10,fig.height=5.5,cache=T}
...
```{r Figure2-24, fig.cap="(ref:fig2-24)",echo=F,fig.width=5,fig.height=5,cache=F}
tibble(Tstar) %>%  ggplot(aes(x = Tstar)) + 
  geom_histogram(aes(y = ..ncount..), bins = 15, col = 1, fill = "skyblue", center = 0) +
  stat_bin(aes(y = ..ncount.., label = ..count..), bins = 15, geom = "text") + 
  geom_density(aes(y = ..scaled..)) + theme_bw() + labs(y = "Density") +
...
```{r Figure2-25, fig.cap="(ref:fig2-25)",echo=F,fig.width=5,fig.height=5,cache=F}
tibble(Tstar) %>%  ggplot(aes(x = Tstar)) + 
  geom_histogram(aes(y = ..ncount..), bins = 15, col = 1, fill = "skyblue", center = 0) +
  stat_bin(aes(y = ..ncount.., label = ..count..), bins = 15, geom = "text") + 
  geom_density(aes(y = ..scaled..)) + theme_bw() + labs(y = "Density") +
  geom_vline(xintercept = quantiles, col = "red", lwd = 2)
