...
par(mfrow=c(1,2))
hist(dd$Distance, xlab="Distance (cm)", labels=T, main="Histogram of Distances", ylim=c(0,1900))
...
library(yarrr)
set.seed(12345)
pirateplot(Distance~Condition,data=dd,inf.method="ci", point.o=0.25)
```
...
```{r Figure2-6, fig.cap="(ref:fig2-6)",echo=F,fig.width=5.75,fig.height=3.75}
par(mfrow=c(1,2))
#old.par <- par(mai=c(0.5,0.5,0.5,0.5))
boxplot(Distance~Condition,data=ddsub) 
pirateplot(Distance~Condition,data=ddsub,inf.method="ci", point.o=0.25)
```
...
```{r Figure2-7, echo=FALSE, fig.cap ="(ref:fig2-7)",out.width="100%"}
...
`r pdata(Tstar,-30,lower.tail=T)*1000` $T^*$ values smaller than -30 based on the 
...
```{r Figure2-12, fig.cap="(ref:fig2-12)",echo=F,out.width="100%"}
...
pandoc.table(x, keep.line.breaks = TRUE,caption="(\\#tab:Table2-1) (ref:tab2-1)", split.table = Inf,
             justify="left")
```
...
```{r Figure2-13, fig.cap="(ref:fig2-13)",echo=F,fig.width=10,fig.height=5}
par(mfrow=c(1,2))
...
```{r Figure2-14, echo=FALSE, fig.cap ="(ref:fig2-14)",fig.width=7,fig.height=4}
...
```{r Figure2-15, fig.cap ="(ref:fig2-15)", echo=F,fig.height=4,fig.width=6}
...
```{r Figure2-16, fig.cap="(ref:fig2-16)",echo=F,fig.width=10,fig.height=5}
par(mfrow=c(1,2))
hist(Tstar, labels=T,ylim=c(0,250))
abline(v=c(-1,1)*Tobs, lwd=2, col="red")
plot(density(Tstar), main="Density curve of Tstar")
...
pirateplot(SimDistance~Condition, data=ddsub)
```
...
```{r Figure2-20, fig.cap="(ref:fig2-20)",echo=F,fig.width=10,fig.height=5,cache=T}

#Simulation study of generating 10 data sets and either using the first or "best p-value" result:
set.seed(1234)

B <- 1000 # # of simulations
Diffmeans <- pvalues <- Diffmeans_Min <- pvalues_Min <- matrix(NA, nrow=B) #To store results
for (b in (1:B)){ #Simulation study loop to repeat process B times
  diff10 <- pval10 <- matrix(NA, nrow=10) #Create empty vectors to store 10 results for each b
...
results <- tibble(pvalue_results=c(pvalues,pvalues_Min), Diffmeans_results=c(Diffmeans, Diffmeans_Min), Scenario = rep(c("First", "Min"), each=B))

par(mfrow=c(1,2)) #Plot results
pirateplot(pvalue_results~Scenario,data=results, inf.f.o = 0,inf.b.o = 0,avg.line.o = 0, main="(a) P-value results")
abline(h=0.05, lwd=2, col="red", lty=2)
pirateplot(Diffmeans_results~Scenario,data=results, inf.f.o = 0,inf.b.o = 0,avg.line.o = 0, main="(b) Estimated difference in mean results")
...
```{r Figure2-21, fig.cap="(ref:fig2-21)",echo=F,warning=F,message=F}
m <- 1:100
pr <- 1-.95^m
...
```{r Figure2-22, fig.cap="(ref:fig2-22)",echo=F,fig.width=5,fig.height=5,cache=T}
...
for (b in (1:B)){ #Simulation study loop to repeat process B times
  pval10 <- matrix(NA, nrow=10) #Create empty vectors to store 10 results for each b
  for (c in (1:10)){ #Loop to create 10 data sets and extract results
    ddsub$SimDistance <- simulate(lm_commonmean)[[1]]
    lm_sim <- lm(SimDistance ~ Condition, data=ddsub) #Estimate two group model using simulated responses
    pval10[c] <- summary(lm_sim)$coef[2,4]
...
#Put results together
results <- tibble(pvalue_results=c(pvalues,pvalues_Min), Scenario = rep(c("First", "Min"), each=B))
...
```{r Figure2-23, fig.cap="(ref:fig2-23)",echo=F,cache=T}
...
```{r Figure2-24, fig.cap="(ref:fig2-24)",echo=F,fig.width=10,fig.height=5,cache=F}
par(mfrow=c(1,2))
hist(Tstar, labels=T,ylim=c(0,330))
...
```{r Figure2-25, fig.cap="(ref:fig2-25)",echo=F,fig.width=10,fig.height=5,cache=F}
par(mfrow=c(1,2))
hist(Tstar, labels=T,ylim=c(0,330))
...
```{r Figure2-26,fig.cap="(ref:fig2-26)",cache=T,echo=F}
par(mfrow=c(1,1))
...
```{r Figure2-27,fig.cap="(ref:fig2-27)",echo=F,fig.width=10,fig.height=5,cache=F}
par(mfrow=c(1,2))
hist(Tstar,labels=T,ylim=c(0,250))
abline(v=Tobs,col="red",lwd=2)
...
* **pirateplot(`r colFmt("Y","red")`~`r colFmt("X","red")`, data=`r colFmt("DATASETNAME","red")`, inf.method="ci")**
