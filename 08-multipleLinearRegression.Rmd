...
\indent To both illustrate the calculation of partial residuals and demonstrate their potential utility, a small simulated example is considered. These are simulated data to help to highlight these patterns but are not too different than results that can be seen in some real applications. This situation has a response of simulated cholesterol levels with (also simulated) predictors of age, exercise level, and healthiness level with a sample size of $n=100$. First, consider the plot of the response versus each of the predictors in Figure \@ref(fig:FigureXX1). It appears that age might be positively related to the response, but exercise and healthiness levels do not appear to be related to the response. But it is important to remember that the response is made up of potential contributions that can be explained by each predictor and unexplained variation, and so plotting the response versus each predictor may not allow us to see the real relationship with each predictor.
...
```{r FigureXX1,fig.cap="Scatterplot of Cholesterol level versus three predictors (simulated data).",echo=T,fig.width=5,fig.height=3}
par(mfrow=c(1,3))
...
* **Be cautious about doing prediction in MLR -- you might be doing extrapolation!**
    ...
    ```{r Figure8-13,fig.cap="Scatterplot of observed Elevations and Maximum Temperatures for SNOTEL data.",echo=F,fig.width=5,fig.height=3}
...
* **Multicollinearity in MLR models:**
    ...
    ```{r Figure8-14,fig.cap="Plot of correlation matrix in the snow depth data set with influential points removed",fig.width=6.5,echo=F}
    library(corrplot)
    par(mfrow=c(1,1), oma=c(0,0,1,0))
...
```{r Figure8-15,fig.cap="(ref:fig8-15)",fig.height=5.5,fig.width=10}
library(openintro)
data(satGPA)
...
```{r Figure8-17,fig.cap="(ref:fig8-17)",warning=F,message=F,echo=F}
plot(allEffects(gpa1, residuals=T))
```
...
```{r Figure8-18,fig.cap="(ref:fig8-18)",fig.height=5,fig.width=10}
...
```{r Figure8-19,fig.cap="(ref:fig8-19)",echo=F,fig.height=5,fig.width=10}
library(viridis)
scatterplot(FYGPA~SATV|GENDER, lwd=3, data=satGPA, smooth=F,
            main="Scatterplot of GPA vs SATV by Sex", cex.main=1, col=viridis(8)[c(7,1)])
```

(ref:fig8-20) Plot of FYGPA vs SATM by Sex of students.

```{r Figure8-20,fig.cap="(ref:fig8-20)",echo=F,fig.height=5,fig.width=10}
scatterplot(FYGPA~SATM|GENDER, lwd=3, data=satGPA, smooth=F,
            main="Scatterplot of GPA vs SATM by Sex", cex.main=1, col=viridis(8)[c(7,1)])
```
...
```{r Figure8-21,fig.cap="(ref:fig8-21)",echo=F,warning=F,message=F}
...
plot(dm1$SATM,fitmales,ylim=c(min(fitmales),max(fitfemales)),
     ...
     type="l",lwd=2,xlab="SATM",ylab="FYGPA", col=viridis(8)[7])
lines(dm1$SATM,fitfemales,lwd=3,col=viridis(8)[1])
...
```{r Figure8-22,fig.cap="(ref:fig8-22)",warning=F,message=F}
...
```{r Figure8-23,fig.cap="(ref:fig8-23)",fig.height=5,fig.width=10}
library(heplots)
library(viridis)
data(Headache)
...
```{r Figure8-24,fig.cap="(ref:fig8-24)",echo=F, fig.height=4,fig.width=7}
...
```{r Figure8-26,fig.cap="(ref:fig8-26)",fig.width=10,fig.height=5}
par(mfrow=c(2,2), oma=c(0,0,2,0))
plot(head1,pch=16,
...
```{r Figure8-27,fig.cap="(ref:fig8-27)",warning=F,message=F,fig.width=8,fig.height=4}
...
```{r Figure8-28,fig.cap="(ref:fig8-28)",echo=F,fig.width=8,fig.height=4.5}
library(smdata)
data("dyslexic3")
...
```{r Figure8-35,fig.cap="(ref:fig8-35)",fig.height=4,fig.width=7,echo=F}
...
```{r Figure8-36,fig.cap="(ref:fig8-36)",echo=F}
...
```{r Figure8-37,fig.cap="(ref:fig8-37)",echo=F,warning=F,message=F,out.width="100%",fig.align="center"}
...
```{r Figure8-38,fig.cap="(ref:fig8-38)",echo=F, fig.height=4, fig.width=8,echo=F,warning=F,message=F}
...
```{r Figure8-39,fig.cap="(ref:fig8-39)",echo=F,fig.height=5,fig.width=10}
library(coneproj)
data(FEV)
FEV <- as_tibble(FEV)
...
```{r Figure8-40,fig.cap="(ref:fig8-40)",fig.height=5,fig.width=10}
...
```{r Figure8-41,fig.cap="(ref:fig8-41)",echo=F, fig.height=7, fig.width=10,echo=F,warning=F,message=F}
...
```{r Figure8-42,fig.cap="(ref:fig8-42)",fig.height=5,fig.width=10}
fm1R$coefficients
vif(fm1R)
confint(fm1R)
